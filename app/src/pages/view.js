...
class View extends Component{
  ...
  componentWillMount(){
    ...
    getDownloadInfo(postId).then((response)=>{
        ...
        console.log("got post info",response.data);
    }).catch((err)=>{
  ...
  render(){
    ...
    <div className={'app-page-download'}>
      <div className={'app-top-header'}>
        <h1 title={'Go back to main site'} onClick={()=>{
          history.push('/');
            ...
            <div className={'app-download-file-list'}>
            {
              files.map((file,index)=>{
                return(<div key={index} className={'app-download-file-list-item'}>
                  ...
                  <div className={'download-action'}><a href={`${apiUrl}/download/${_.get(file,'_id')}`}><i className={'icon-download'} /></a></div>
              </div>);
              })
            }
